...
class checkinController extends Controller
{
   ...
   **/
   public function add_to_cart(Request $request, $checkinCode){
      ...
      //                               ->where('product_information.business_code',$checkin->customer_id)
      //                               ->first();

      // $checkInCart = Cart::where('checkin_code',$checkinCode)->where('productID',$request->productID)->count();

      // if($checkInCart > 0){
      //    $cart = Cart::where('checkin_code',$checkinCode)->where('productID',$request->productID)->first();
      ...
      //    $cart->userID = Auth::user()->user_code;
      //    $cart->save();
      // }else{
      //    $cart = new Cart;
